...
function createWindowsTerminal() {
  ...
  const rpc = createRPC(message => {
    if (!child) {
      child = spawn('powershell.exe', ['-NoLogo', '-NoProfile', '-NonInteractive', '-ExecutionPolicy', 'Bypass', '-File', path.join(__dirname, 'windows.ps1').replace(/app\.asar([\\/])/, 'app.asar.unpacked$1')], { stdio: ['pipe', 'pipe', 'ignore'], windowsHide: true })
      buffer = ''
      child.on('error', () => { rpc.disconnect(); child = null })
      child.on('close', () => { rpc.disconnect(); child = null })
