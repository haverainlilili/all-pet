...
版本：v1.4.0；记录日期：2026-09-26。本文随主程序仓库保存，描述发布版本的实际实现、用户操作与边界。开发中的会话导出与独立生成器不随此版本发行，相关记录单独标明。

发布变更见 [CHANGELOG](../CHANGELOG.md)，安装及测试结果见 [v1.4.0 发布说明](releases/v1.4.0.md) 与 [平台行为验收](平台行为验收.md)。
...
| 安装包构建 | Electron + Swift sidecar；DMG / ZIP / AppImage / DEB / NSIS | v1.4.0 发布目标；构建结果见版本发布说明，不能替代各系统实机验收 |
| 自动更新 / 云同步 | 未见实现 | 未实现 |

会话导出属于独立开发内容，未纳入 v1.4.0 的源码提交或安装包；下文保留设计记录，不把它作为可用命令。
...
macOS Electron 优先启动同包 `menu-bridge` AppKit 进程，以原生 `NSMenuItem.view` 复用连续尺寸控件；桥接失败回退 Electron 托盘菜单。Windows/Linux 保留 Electron 原生托盘，另有“宠物管理…”窗口。macOS 点击状态栏图标打开菜单，Windows/Linux 托盘主点击切换显示。Electron 采用单实例锁，关闭窗口不等于退出，退出时清理 watcher、菜单桥接、计时器和缩略图子进程。
...
**当前源码内置 5 只**，按默认排序为 Boba、Tiko、团团和米粒、Hoops、西瓜🍉（`boba`、`tiko`、`cat-hamster-duo`、`hoops`、`watermelon`）。旧 README 的“4 只”已经落后于当前源码。
...
### 11.3 历史会话索引与文本导出（设计记录，不随 v1.4.0 发布）
...
| Windows/Linux 托盘图标 | Electron 托盘存在 | 单击 | 切换宠物显示/隐藏 | 仅进程内窗口状态 | 右键使用已注册的上下文菜单；菜单图像缩略图仅 macOS 分支设置，不能承诺 Windows/Linux 菜单宠物带图 |
...
| Windows/Linux 大小菜单项 | 已有宠物、未忙 | 点击减小/增大 | 同样调整 scale；没有自动重新弹出菜单的处理 | 写 `pet.scale` | 到边界不再变化；不因到边界禁用按钮 |
...
| 宠物子菜单已安装项 | fallback 或 Windows/Linux，非当前项、未忙 | 点击宠物项 | 发出切换命令，桌面更新；普通菜单项正常结束本次菜单操作，不自动重开 | 写默认宠物路径 | 当前项显示勾选且禁用；无有效目标路径、忙时也禁用；空列表显示禁用的“暂无已安装宠物” |
| 已安装行右侧 × | macOS bridge，菜单状态未忙 | 点击 × | 先关闭子菜单，然后弹出 Electron 原生删除确认框；红色悬停提示 | 确认前无 | 删除当前项也允许；行主体与删除命中区分开，不先切换宠物 |
| 宠物 → 删除宠物… → 某项 | fallback 或 Windows/Linux，至少一项已安装、未忙 | 选择要删除的宠物 | 弹出原生删除确认框，包含名称、路径、“不可撤销” | 确认前无 | 无已安装宠物或忙时整个删除子菜单禁用；macOS bridge 用行内 ×，没有该独立删除子菜单 |
...
macOS bridge 菜单打开时，百分比、大小可用性与图标提示可立即更新，宠物条目/勾选和普通菜单结构重建推迟到菜单关闭。旧条目若仍可点击，主进程操作闸门仍会拒绝另一项正在进行的宠物变更；这并不等于所有正在展开的条目立即视觉禁用。fallback / Windows / Linux 使用普通 Electron 菜单，不具有 bridge 自定义行的“点击主体后保持菜单打开”行为。
...
| 点击一个已勾选平台 | 取消勾选，立即从收起卡、平台列表和会话列表移除该平台；若正在该平台会话层则返回平台层；macOS 菜单保持展开 | 保存 hiddenBubblePlatforms；后台仍扫描，保留历史、已确认记录和监控配置 |
| 点击一个未勾选平台 | 重新勾选，当前活动和未过期/未确认历史重新可见；macOS 菜单保持展开 | 重启仍保留；已点击确认或过期的通知不会因重新勾选复活 |
| 全部隐藏 | 九项原位取消勾选；气泡消失，宠物仍保留；macOS 菜单保持展开 | 不终止原平台任务、不清空历史；宠物状态动画按允许展示的平台重新计算，可回空闲 |
| 全部显示 | 原位恢复九项勾选；有通知时显示气泡，没有通知时仍不凭空显示卡片；macOS 菜单保持展开 | 原平台监控 enabled=false 的设置不被覆盖；无快照的平台仍不会新增实际任务 |
