...
本节记录当前源码；v1.4.1 安装包仍保持原发布行为。下文原生 AppKit GUI 的专属能力和旧版本说明另列。

| 操作 | 可见结果 | 菜单是否关闭 |
| --- | --- | --- |
| 三平台左/右键托盘 | 打开同一套 Mac 风格菜单，只有一个托盘图标 | 打开或切换 |
| 悬停/点击宠物、气泡显示平台、实时状态接入 | 侧边展开，保留主菜单；屏幕边缘反向展开 | 不关闭 |
| 单平台勾选、全部显示/隐藏 | 更新勾选和气泡过滤，保留监控及历史 | 不关闭 |
| 行内 − / ＋ | 原始 scale 每次改变 0.05，显示约改变 9 个百分点，刷新百分比 | 不关闭 |
| 点击已安装宠物 | 切换宠物并刷新标记和缩略图 | 不关闭 |
| 显示/隐藏宠物、刷新目录 | 更新可见性或目录 | 不关闭 |
| 删除宠物 | 打开不可撤销操作确认；取消不删除 | 关闭 |
| 终端接入说明与扩展 | 打开随包说明，包含 VSIX/Lua 配置与位置 | 关闭 |
| Esc、点击其它窗口 | 收起菜单 | 关闭 |
| 上下/左右/Enter | 遍历、进入/返回子菜单、执行当前动作 | 依动作而定 |

终端任务的调用链和表现：

1. 每秒捕获任务对应的原终端，不等待第一次点击。Mac 无菜单桥接、Windows 常驻辅助进程、Linux `/proc` 分别提供进程身份；绑定随任务历史保存。
2. 身份来自日志持有进程、精确会话参数或带启动时间核验的进程记录。绑定包含 shell PID、启动时间及 TTY/原控件；Linux 加 boot ID。目录和普通标题不作为任务身份。
3. 点击完成/失败气泡先确认并保存，然后定位已有终端。失败说明原因；不会执行 CLI、resume 或开新标签页冒充原任务。运行中气泡保持。
4. 手动进入后由独立 500 ms 循环检查前台与对应 tab/pane。确认只作用于被检查的完成轮次；迟到回复不能清掉新一轮。
5. CLI 退出、shell 仍在时绑定有效；shell/窗口关闭、PID/TTY 复用、来源变化会失效。同一终端已承载更新任务时，不确认旧任务。
6. RPC 带请求 ID、超时、断连清理；Mac 桥接退出后恢复。渲染页面不接收终端控制命令或私有 locator。
7. 终端接口不同，配置和限制以 [接入矩阵](../desktop/integrations/README.md) 为准；适配器存在不表示每个终端版本都已实测。
8. VS Code 的 activeTerminal 不能证明面板可见。扩展只认切换终端事件或“已查看当前终端任务”命令；重新显示原本已选中的面板不保证自动确认。WezTerm 状态超过 1.2 秒即丢弃。
9. Mac 桌面 Codex/Claude/DSH 的识别与权限入口保留；菜单统一不等于系统权限相同。
...
Electron 依赖声明为 Electron `^44.2.0`、electron-builder `^26.15.3`，应用包版本字段为 `1.4.1`。这些是本地配置值，不是“最新官方版本”声明。
...
Cursor、WorkBuddy、Qoder、Z Code 尚无已验证的当前任务识别，手动进入任务后自动清除**仍未实现**。Windows/Linux 的 GUI 会话不因此获得上述 macOS 检测能力；开发版 CLI 任务通过终端适配器独立识别。九平台的点击确认规则已统一，但手动查看检测不能宣称九平台一致。详细实测与缺口见 [平台行为验收](平台行为验收.md)。
...
- 开发版 Grok 与 CLI 使用终端适配器；失败说明原因，不自动创建新任务，配置要求见本文开发版补充。
- 应用启动器接受请求只能证明请求已发出，不等于目标会话已显示。`exact`、`openedApp`、通知已确认分别表达不同结果。
- Cursor / WorkBuddy / Qoder / Z Code 在 macOS 发出应用打开请求并明确 `exact=false`；Windows/Linux 提示限制并提供“只打开平台”。pi 任务使用终端适配器定位原会话，不自动启动重复会话。
...
开发版 Electron 三平台共用持续级联菜单；macOS 的同包 `menu-bridge` 后台运行，只提供系统识别与权限入口。桥接失败不改变菜单样式，后台按 2～30 秒退避恢复。另有“宠物管理…”窗口；三平台左/右键均可打开菜单。Electron 采用单实例锁，关闭窗口不等于退出，退出时清理 watcher、菜单桥接、计时器和缩略图子进程。
...
核对日期：2026-09-26。依据当前源码逐项核对，记录实际行为及当前限制；不含气泡/精灵事件。本节中的“管理窗”指 Electron 的 `AllPet 宠物管理` 窗口；开发版三平台使用统一菜单，macOS bridge 仅提供后台系统识别。
...
### 15.9 Electron 托盘入口、菜单与平台差异（当前开发版）

| 入口 / 操作 | 前置条件 | 可见反馈与动作 | 持久化与边界 |
| --- | --- | --- | --- |
| 左/右键托盘图标 | 三平台 Electron 运行 | 打开同一套 Mac 风格级联菜单，靠屏幕边缘时反向展开子菜单 | 不直接隐藏宠物；无额外 Mac bridge 图标 |
| 悬停或点击子菜单 | 宠物、气泡显示平台、实时状态接入 | 约 120 ms 后展开；主菜单保留，子菜单可连续点击 | 小屏幕退化为单页并提供返回按钮 |
| 显示/隐藏宠物 | 主窗口存在或可重建 | 切换窗口可见性，托盘与监控继续 | 不写 `pet.enabled`；保持菜单打开，无有效宠物时按原窗口恢复规则处理 |
| 内嵌大小 − / ＋ | 有宠物且未忙 | 原始 scale ±0.05，百分比原位刷新；中间百分比无动作 | 范围 0.4～1.2，显示步长约 9 个百分点；连续操作不关闭 |
| 已安装宠物行 | 非当前项且未忙 | 切换宠物、更新 ✓ 与缩略图 | 保存选择；当前项禁用；保持打开 |
| 行内删除 × | 未忙 | 先关菜单再弹确认，取消不删除 | 确认后按精确 bundle 路径删除；成功刷新并按规则回退 |
| 未安装默认宠物 | 未忙 | 关菜单后运行安装流程 | 使用串行闸门，失败显示原因 |
| 宠物管理 / GitHub 安装 / 本地导入 | 未忙 | 关菜单，打开或复用管理窗，按入口选择动作 | 本地多格式导入仍仅 macOS 可用；不是菜单样式差异 |
| 刷新宠物目录 | 未忙 | 更新目录及当前选择，菜单保持打开 | 失败提示；不会创建并发安装操作 |
| 平台开关 / 全部显示隐藏 | 九个平台 | 即时更新勾选与气泡过滤，主/子菜单保持打开 | 保存 hiddenBubblePlatforms，不删除历史，不停止监控 |
| 终端接入说明与扩展 | 无 | 关闭菜单并打开随包说明 | 包含 VSIX、WezTerm Lua、kitty/Linux 配置与支持边界 |
| 自动确认权限入口 | macOS | 显示授权状态，点击通过后台桥接打开系统权限入口 | 系统授权需本人确认；Windows/Linux 不显示 Mac 专属权限项 |
| 打开配置 | 无 | 关菜单，打开配置文件 | 外部编辑按原生配置刷新规则生效 |
| 退出 | 无 | 关闭窗口、托盘、watcher、系统桥接、终端辅助进程和定时器 | 已保存配置及历史保留 |
| Esc / 失焦 / 点击透明空隙 | 菜单打开 | 收起菜单 | 不修改设置 |
| 非法 IPC / 已禁用动作 | 来自菜单页面的请求仍需校验 | 拒绝未知平台、目录外路径、非当前 catalog 对象 | 不执行请求；受限 preload，不向页面提供 shell 权限 |

旧 AppKit 菜单构造仍保留在专门的诊断 smoke 入口，不是当前 Electron 产品菜单。独立原生 AppKit GUI 的菜单行为见 15.7。
...
| 点击一个已勾选平台 | 取消勾选，立即从收起卡、平台列表和会话列表移除该平台；若正在该平台会话层则返回平台层；三平台 Electron 菜单保持展开 | 保存 hiddenBubblePlatforms；后台仍扫描，保留历史、已确认记录和监控配置 |
| 点击一个未勾选平台 | 重新勾选，当前活动和未过期/未确认历史重新可见；三平台 Electron 菜单保持展开 | 重启仍保留；已点击确认或过期的通知不会因重新勾选复活 |
| 全部隐藏 | 九项原位取消勾选；气泡消失，宠物仍保留；三平台 Electron 菜单保持展开 | 不终止原平台任务、不清空历史；宠物状态动画按允许展示的平台重新计算，可回空闲 |
| 全部显示 | 原位恢复九项勾选；有通知时显示气泡，没有通知时仍不凭空显示卡片；三平台 Electron 菜单保持展开 | 原平台监控 enabled=false 的设置不被覆盖；无快照的平台仍不会新增实际任务 |
...
统一菜单通过受限 preload 发送 `bubble-platform-toggle` / `bubble-platform-all` / `integration-install`，Electron 主进程校验并保存设置、返回最新状态。勾选失败不冒充保存成功，Esc/失焦关闭。独立原生 AppKit GUI 使用自己的内嵌勾选控件，配置键及九平台名称顺序相同。
