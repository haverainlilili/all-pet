...
| 8 | 完成卡片常驻 | done 终态卡片 + 轮播堆栈 + 「+N」 | ✅ 完成卡片 + 「+N」已对齐，点击查看后自动消失；轮播堆栈简化为平铺 |
...
> - 任务完成后的气泡清理：Electron 在平台转入 idle 时清理历史里的活跃任务（避免「已完成但显示运行中/等待中」的僵尸残留）；macOS 靠「手动查看后自动 dismiss」+ 12 条上限逐步收敛。
